...
namespace Day5
{
    class Program
    {
        ...
        static void PartTwo()
        {
            ...
            var pointer = 0;
            var steps = 0;
            while (pointer < workingArray.Length || pointer < 0)
            {
                var nextMove = workingArray[pointer];
                if (nextMove >= 3)
                    workingArray[pointer]--;
