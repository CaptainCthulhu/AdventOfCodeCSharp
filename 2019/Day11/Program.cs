...
namespace Day11
{
    class Program
    {
        ...
        static List<Instruction> instructions = new List<Instruction> { new Instruction(0, 2) };
        ...
        {
            Interop.OpCodeMachine(input, Instructions);
        ...
        public class Instruction
        {
            ...
            public Instruction(long color, long direction)
            {
                location = DetermineLocation();
                paintedSquares.Add(new PaintedSquare(color, location));
            }
        }

        static Point DetermineLocation()
        {
            if (instructions.Count > 0)
            {
                var previousLocation = instructions.Last();
                if (previousLocation.facing == "Up")
                {
                    return new Point(previousLocation.location.X, previousLocation.location.Y + 1);
                }
                ...
                {
                    return new Point(previousLocation.location.X - 1, previousLocation.location.Y);
                }
                if (previousLocation.facing == "Right")
                {
                    return new Point(previousLocation.location.X + 1, previousLocation.location.Y);
                }
            }
        ...
        public class PaintedSquare
        {
            ...
            public PaintedSquare(long color, Point location)
            {
                this.color = color;
        ...
        static int Instructions(Queue<long> outputs)
        {
            while (outputs.Count >= 2)
            {
                ...
                instructions.Add(new Instruction(color, direction));
            }

            if (paintedSquares.Contains(x => x.location == instructions.Last().location))
                {
